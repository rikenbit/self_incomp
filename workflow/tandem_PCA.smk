...
rule all:
    input:
        expand('output/LOOCV_rf/tandem_PCA_{DIM}.csv', DIM=dimention,)
...
rule u_models:
    input:
        'output/inputTensors.RData'
    output:
        'output/X_Tensor/tandem_PCA_{DIM}.RData',
        'output/X_Tensor/tandem_PCA_{DIM}.csv'
    benchmark:
        'benchmarks/X_Tensor/tandem_PCA_{DIM}.txt'
    container:
        'docker://yamaken37/shuffled_pca:20230801'
    resources:
        mem_gb=200
    log:
        'logs/X_Tensor/tandem_PCA_{DIM}.log'
    shell:
        'src/tandem_PCA.sh {input} {output} {wildcards.DIM}'
...
rule SSI_scikit_rf:
    input:
        ...
        'output/y_tandem.csv'
    output:
        'output/LOOCV_rf/tandem_PCA_{DIM}.csv'
    benchmark:
        'benchmarks/LOOCV_rf/tandem_PCA_{DIM}.txt'
    container:
        "docker://yamaken37/ssi_sklearn_env:202212141249"
    resources:
        mem_gb=200
    log:
        'logs/LOOCV_rf/tandem_PCA_{DIM}.log'
    shell:
