...
rule all:
    input:
        expand('output/toy/vis_common_factors/{list_l}.png', list_l=list_LOOCV)
rule SSI_Vis_train:
    output:
        'output/toy/vis_common_factors/{list_l}.png'
    params:
        input_dir = 'output/toy/MT_train_X/tensor',
        input_pattern = '{list_l}'
    benchmark:
        'benchmarks/output/toy/vis_common_factors/{list_l}.txt'
    container:
        "docker://yamaken37/cfa_180row:20230516"
    resources:
        mem_gb=200
    log:
        'logs/output/toy/vis_common_factors/{list_l}.log'
    shell:
        'src/SSI_cfa_180row.sh {params} {output} >& {log}'


# # SSI_cfa_180row Bra x Bra
# ###################################################
# # list_LOOCV = [
# # 'MODELS_Model-1-A1_r1_20_r2_100_r3_5_r1L_xx_r1R_xx_r2L_xx_r2R_xx_r3L_xx_r3R_xx'
# # ]
