...
rule all:
    input:
        # expand('output/LOOCV_LDA/{list_l}.csv', list_l=list_LOOCV),
        # expand('output/OneDim_LDA/{list_l}.csv', list_l=list_LOOCV)
        expand('output/OneDim_LDA/{list_l}.svg', list_l=list_LOOCV)

rule SSI_scikit_LDA_py:
    input:
        'output/X_Tensor/{list_l}.csv',
        'output/SSI/y_r.csv'
    output:
        'output/LOOCV_LDA/{list_l}.csv',
        'output/OneDim_LDA/{list_l}.csv'
    benchmark:
        'benchmarks/LOOCV_LDA/{list_l}.txt'
    container:
        "docker://yamaken37/ssi_sklearn_env:202212141249"
    resources:
        mem_gb=200
    log:
        'logs/LOOCV_LDA/{list_l}.log'
    shell:
        'source .bashrc && conda activate sklearn-env && python src/SSI_scikit_LDA.py {input} {output} >& {log}'

rule SSI_scikit_LDA:
    input:
        'output/OneDim_LDA/{list_l}.csv'
    output:
        'output/OneDim_LDA/{list_l}.svg'
    params:
        'output/SSI/y_r.csv'
    benchmark:
        'benchmarks/OneDim_LDA/{list_l}.txt'
    container:
        "docker://yamaken37/heatmap_svg:2023013118"
    resources:
        mem_gb=200
    log:
        'logs/OneDim_LDA/{list_l}.log'
    shell:
        'src/SSI_scikit_LDA.sh {input} {output} {params} >& {log}'