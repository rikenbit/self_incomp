...
rule all:
    input:
        expand('output/Tensor_scale_Train/test_X/predict/{params}.csv', params = paramspace.instance_patterns)
...
rule train_u_models:
    input:
        'data/train_Tensors.RData'
    output:
        expand('output/Tensor_scale_Train/train_X/tensor/{params}.RData', params = paramspace.wildcard_pattern),
        expand('output/Tensor_scale_Train/train_X/tensor/{params}.csv', params = paramspace.wildcard_pattern),
        expand('output/Tensor_scale_Train/train_X/one_slice_tensor/{params}.RData', params = paramspace.wildcard_pattern)
    params:
    ...
    benchmark:
        f'benchmarks/Tensor_scale_Train/train_X/tensor/{paramspace.wildcard_pattern}.txt'
    container:
        'docker://koki/tensor-projects-self-incompatible:20221217'
    resources:
        mem_gb=200
    log:
        f'logs/Tensor_scale_Train/train_X/tensor/{paramspace.wildcard_pattern}.log'
    shell:
...
rule SSI_scikit_rf_fit_MT:
    input:
        ...
        'output/SSI/y_r.csv'
    output:
        expand('output/Tensor_scale_Train/train_X/fit/{params}.pickle', params = paramspace.wildcard_pattern)
    params:
        args10 = lambda w: w["row"]
    benchmark:
        f'benchmarks/Tensor_scale_Train/train_X/fit/{paramspace.wildcard_pattern}.txt'
    container:
        "docker://yamaken37/ssi_sklearn_env:202212141249"
    resources:
        mem_gb=200
    log:
        f'logs/Tensor_scale_Train/train_X/fit/{paramspace.wildcard_pattern}.log'
    shell:
...
rule test_u_models:
    input:
        ...
        expand('output/Tensor_scale_Train/train_X/tensor/{params}.RData', params = paramspace.wildcard_pattern)
    output:
        expand('output/Tensor_scale_Train/test_X/tensor/{params}.csv', params = paramspace.wildcard_pattern)
    params:
        args0 = lambda w: w["MODELS"]
    benchmark:
        f'benchmarks/Tensor_scale_Train/test_X/tensor/{paramspace.wildcard_pattern}.txt'
    container:
        'docker://koki/tensor-projects-self-incompatible:20221217'
    resources:
        mem_gb=200
    log:
        f'logs/Tensor_scale_Train/test_X/tensor/{paramspace.wildcard_pattern}.log'
    shell:
...
rule SSI_U_Predict:
    input:
        ...
        expand('output/Tensor_scale_Train/test_X/tensor/{params}.csv', params = paramspace.wildcard_pattern)
    output:
        expand('output/Tensor_scale_Train/test_X/predict/{params}.csv', params = paramspace.wildcard_pattern)
    benchmark:
        f'benchmarks/Tensor_scale_Train/test_X/predict/{paramspace.wildcard_pattern}.txt'
    container:
        "docker://yamaken37/ssi_sklearn_env:202212141249"
    resources:
        mem_gb=200
    log:
        f'logs/Tensor_scale_Train/test_X/predict/{paramspace.wildcard_pattern}.log'
    shell:
