...
rule all:
    input:
        expand('output/PCA/MT_test_X/predict/{params}.csv', params = paramspace.instance_patterns)
...
rule train_u_models:
    input:
        'data/train_Tensors.RData'
    output:
        expand('output/PCA/MT_train_X/tensor/{params}.RData', params = paramspace.wildcard_pattern),
        expand('output/PCA/MT_train_X/tensor/{params}.csv', params = paramspace.wildcard_pattern),
        expand('output/PCA/MT_train_X/one_slice_tensor/{params}.RData', params = paramspace.wildcard_pattern)
    params:
...
rule SSI_scikit_rf_fit_MT:
    input:
        ...
        'output/y_r.csv'
    output:
        expand('output/PCA/MT_train_X/fit/{params}.pickle', params = paramspace.wildcard_pattern)
    params:
        args10 = lambda w: w["row"]
    benchmark:
...
rule test_u_models:
    input:
        ...
        expand('output/PCA/MT_train_X/tensor/{params}.RData', params = paramspace.wildcard_pattern)
    output:
        expand('output/PCA/MT_test_X/tensor/{params}.csv', params = paramspace.wildcard_pattern)
    params:
        args0 = lambda w: w["MODELS"]
    benchmark:
...
rule SSI_U_Predict:
    input:
        ...
        expand('output/PCA/MT_test_X/tensor/{params}.csv', params = paramspace.wildcard_pattern)
    output:
        expand('output/PCA/MT_test_X/predict/{params}.csv', params = paramspace.wildcard_pattern)
    benchmark:
