...
args_input_pattern <- args[1]

args_input_dir <- args[2]
args_input_dir2 <- args[3]

args_rec_error <- args[4]
args_rec_error_F <- args[5]
args_rel_change <- args[6]
args_rel_change_F <- args[7]
...
#### rec_error####
# purrr
purrr::map_dfc(cv_sort, .df_rec_error) |>
    rownames_to_column("order") -> df_rec_error
# pivot_longer
df_rec_error |>
    pivot_longer(!order,
                 names_to = "Pullout_row",
                 values_to = "RecError") -> df_rec_error_long

row_order <- c()
for (i in cv_sort) {
    eval(parse(text=paste0("row_order <- c(row_order,'row_",i,"')")))
}
order_order <- df_rec_error$order

gg_rec_error <- ggplot(df_rec_error_long,
...
#### rec_error_F####
# filter
df_rec_error_long |> dplyr::filter(order!="1") -> df_rec_error_long_F

gg_rec_error_F <- ggplot(df_rec_error_long_F,
                         aes(x = factor(order, levels = order_order),
                             y = RecError,
                             color = factor(Pullout_row, levels = row_order)
                         )
    ...
    scale_y_log10()
#### ggsave rec_error_F####
ggsave(filename = args_rec_error_F,
       plot = gg_rec_error_F,
       dpi = 100,
       width = 30.0,
       height = 30.0,
...
gg_rel_change_F <- ggplot(df_rel_change_long_F,
                          aes(x = factor(order, levels = order_order),
                              y = RelChange,
                              color = factor(Pullout_row, levels = row_order)
                              )
